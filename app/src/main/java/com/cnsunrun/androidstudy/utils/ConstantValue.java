package com.cnsunrun.androidstudy.utils;

/**
 * Created by ZhouBin on 2017/6/9.
 * Effect:  存放常量的类
 */

public interface ConstantValue {


    //    banner滚动时间
    int VP_TURN_TIME = 5000;

    String TITLE = "title";

    //支付宝支付
    String ALI_APPID = "2017061607501207";
    String SELLER_ID = "2088221726008512";
    String RSA_PRIVATE2 = "";

    //微信支付
    String WX_APP_ID = "wx7da3be6fb53cc23d"; //微信app_id
    String PARTNER_ID = "1483873202";//商户号
    String API_KEY = "98571771c5147a1b04b8fc9306f35311";
    String WX_ORDER_CALLBACK = "http://test.cnsunrun.com/shunshou//Api/Pay/Callback/weixin_callback";


}
